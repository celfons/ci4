...
(function ($) {
    ...
    Datagrid.prototype.SearchAndOrderingTrigger = function () {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                datagrid_object.hideShowPrintButton();
    ...
    Datagrid.prototype.hideShowPrintButton = function () {

        if (this.gcrud_container.find('.select-row:checked').length > 0) {
            this.gcrud_container.find('.print-selected-button').removeClass('hidden');
        } else {
            this.gcrud_container.find('.print-selected-button').addClass('hidden');
        }
    };
                    ...
                    $.ajax({
                        ...
                        success: function (output) {
                            ...
                            if (output.success) {
                                ...
                                $.growl(output.error_message || 'Registro não foi excluído, tente novamente.', {
                                    type: 'danger',
                                    delay: 10000,
                                    animate: {
                                        enter: 'animated bounceInDown',
